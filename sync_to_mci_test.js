    ...
    for (var i = 0; i < wardId.length; i++) {
        ...
        await axios({
            ...
            .then(async function (response) {
                ...
                logger.info('Begin to make [DID] Socket Data in the ['+wardId[i] +'] department');
                // did 정보 담기
                ...
                console.log("STEP@. DB에 디바이스 데이터(MCI 정보) 업데이트하기")
                for(var j = 0 ; j < patientInfoData.length ; j ++){
                    // esl 정보 담기
...
async function requestDeviceDataApply(id) {
    ...
    await Delay(1000*20,result.name);
